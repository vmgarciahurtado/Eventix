...
void main() {
  ...
  setUp(() {
    ...
    // Se mantiene una suscripción viva: el provider es autoDispose y sin ella
    // el estado se recicla entre lecturas.
    container.listen(
      purchaseProvider,
  ...
  group('finishPayment', () {
    ...
    test('pago cobrado sin reserva confirmada NO libera el cupo', () async {
      ...
      // Hay dinero cobrado: borrar la reserva borraría el rastro que soporte
    ...
    test('un fallo al liberar el cupo no tapa el resultado del pago', () async {
      ...
      // El pending expira solo en 15 min: al usuario se le informa lo que pasó
      // con su pago, no un error de limpieza interna.
      expect(state(), isA<PurchaseNotPaid>());
    });
  });
}
